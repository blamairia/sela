...
class EnforceApiTokenTimeout
{
    ...
    private function unauthorizedWithTokenClear(string $message, string $status, Request $request)
    {
        ...
        // Prevent frontend redirect loops: also clear the "Sela_token" cookie (used by route guards).
        ...
        if ($domain) {
            $response->headers->setCookie(cookie('Sela_token', '', -2628000, '/', $domain));
        }
        $response->headers->setCookie(cookie('Sela_token', '', -2628000, '/'));
