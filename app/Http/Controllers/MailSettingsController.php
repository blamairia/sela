...
class MailSettingsController extends BaseController
{
    ...
    public function test_config_mail(Request $request)
    {
        ...
        try {
            Mail::raw('This is a test email to verify your mail configuration in Sela.', function ($message) use ($to, $settings, $server) {
                $message->to($to)
                ...
                if ($fromEmail) {
                    $fromName = ($server && $server->sender_name) 
                        ? $server->sender_name 
                        : ($settings && $settings->CompanyName ? $settings->CompanyName : 'Sela');
